...
def upgrade() -> None:
    ...
    for name, values in [
        ...
        postgresql.ENUM(*values, name=name, create_type=False).create(op.get_bind(), checkfirst=True)
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('uploaded', 'processing', 'completed', 'failed', name='uploadstatus'), nullable=False, server_default='uploaded'),
    ...
    op.create_table(
        ...
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id'), nullable=False),
        sa.Column('task_type', sa.Enum('3dgs_training', 'door_alignment', 'basemap_realign', name='tasktype'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('pending', 'approved', 'rejected', 'superseded', name='basemapstatus'), nullable=False, server_default='pending'),
    ...
    op.create_table(
        ...
        sa.Column('type', sa.Enum('task_complete', 'task_failed', 'basemap_pending', 'system', name='notificationtype'), nullable=False),
